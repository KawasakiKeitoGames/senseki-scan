...
window.Vision = (() => {
  ...
  function classify(img) {
    ...
    // winner: 黄色ハイライト(勝者行は上下どちらもありうるので両行域) + 半透明パネル(明るい背景のコートでは薄まるので閾値は緩め)
    if (frac(img, 160, 78, 176, 98, isYellow) > 0.06 && frac(img, 148, 77, 173, 98, isOlive) > 0.25) return 'winner';
  ...
  function matchGlyph(g, templates) {
    ...
    for (const t of templates) {
      ...
      s *= 0.7 + 0.3 * ar;
      if (!best || s > best.score) best = { label: t.label, score: s };
    }
    return best;
  }
  ...
  function readGlyphs(img, mode, templates, opts) {
    ...
    let glyphs = boxes.map(b => {
      ...
      return { box: b, glyph: g, label: m2 ? m2.label : null, score: m2 ? m2.score : 0 };
    });
    ...
    if (templates && templates.length && (!opts || !opts.expected)) {
      glyphs = glyphs.flatMap(g => {
        ...
        const two = sp.map(b => {
          ...
          return { box: b, glyph: gg, label: m2.label, score: m2.score };
        });
  ...
  async function scan(video, { step = 1.0, onProgress, albumTpl } = {}) {
    ...
    for (let t = 0.2; t < video.duration; t += step) {
      ...
      if (cls === 'winner' && findBanner(video)) cls = 'other';
  ...
  async function findWinnerFrame(video, win) {
    ...
    for (let t = Math.max(0, win.t0 - 0.5); t <= win.t1 + 0.5; t += 0.25) {
      ...
      if (score > 0.05 && findBanner(video)) continue; // 黄色リボンバナーの誤検出除け
  ...
  async function locateWinner(video, t0, t1, panelFont) {
    const seekTo = makeSeeker(video);
    for (let t = t1; t >= Math.max(0.2, t0); t -= 0.25) {
      ...
      if (Math.max(y1, y2) > 0.15 && Math.min(y1, y2) < 0.08) {
        if (findBanner(video)) continue; // 黄色リボンバナー(サンボショット等)の誤検出除け
        if (panelFont && panelFont.length) {
          const winRegion = y1 >= y2 ? REGIONS.w1digit : REGIONS.w2digit;
  ...
  function groupMatches(events) {
    ...
    for (const e of events) {
      if (e.type === 'vs') matches.push({ vs: e });
